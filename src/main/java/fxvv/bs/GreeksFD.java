...
public class GreeksFD {
    ...
    private static double firstDerivativeAdaptive(DoubleUnaryOperator f, double x, double h0, double lowerBound) {
        ...
        for (int i = 0; i < MAX_REFINEMENTS; i++) {
            ...
            double currRichardson = richardsonExtrapolate(prev, curr, 2);
            if (isStable(prevRichardson, currRichardson)) return currRichardson;
            prev = curr;
            prevRichardson = currRichardson;
            h = hNext;
        }
        return prevRichardson;
    }
    ...
    private static double secondDerivativeAdaptive(DoubleUnaryOperator f, double x, double h0, double lowerBound) {
        ...
        int prevOrder = secondDerivativeOrder(x, h, lowerBound);
        double prevRichardson = prev;
        ...
        for (int i = 0; i < MAX_REFINEMENTS; i++) {
            ...
            int currOrder = secondDerivativeOrder(x, hNext, lowerBound);
            int order = Math.min(prevOrder, currOrder);
            double currRichardson = richardsonExtrapolate(prev, curr, order);
            if (isStable(prevRichardson, currRichardson)) return currRichardson;
            prev = curr;
            prevOrder = currOrder;
            prevRichardson = currRichardson;
    ...
    private static double richardsonExtrapolate(double coarse, double fine, int order) {
        double factor = Math.pow(2.0, order);
